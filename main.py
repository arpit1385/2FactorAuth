...
def get_totp_token(secret):
	...
	return get_hotp_token(secret, intervals_no=int(time.clock_gettime(time.CLOCK_REALTIME))//30)


class OTPWindow(Gtk.Window):

	def __init__(self):
		Gtk.Window.__init__(self, title="2 Factor Authentication")
		...
		runat = int(time.clock_gettime(time.CLOCK_REALTIME)+30)//30*30
		GObject.timeout_add(1000 * (runat - time.clock_gettime(time.CLOCK_REALTIME)), self.update_otps, None)

		runat = int(time.clock_gettime(time.CLOCK_REALTIME)) + 1
		GObject.timeout_add(1000 * (runat - time.clock_gettime(time.CLOCK_REALTIME)), self.update_progress, None)

		self.progressbar_fraction -= (int(time.clock_gettime(time.CLOCK_REALTIME))%30)/30.0
	...
	def update_progress( self, user_data ):
		...
		if self.progressbar_reset == True:
			self.progressbar_fraction = 1 - (int(time.clock_gettime(time.CLOCK_REALTIME))%30)/30.0
			self.progressbar.set_fraction(self.progressbar_fraction)
	...
	def save( self ):
		...
		with open( fullpath( 'py2fa.json' ), 'wb' ) as f:
			f.write(saveJsonText.encode('utf-8'))
	...
	def load( self, accountsList=None ):
		...
		for name, secret in sorted(list(accountsList.items())):
